...
games.sort { $0.name?.localizedCaseInsensitiveCompare($1.name ?? <#default value#>) == .orderedAscending }
...
class KolukuluriHomeViewController: UIViewController,UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        //create a cell
        let cell = KolukuluriTVOL.dequeueReusableCell(withIdentifier: "KolukuluriCell", for: indexPath)
        //populate the cell
    ...
    @IBOutlet var KolukuluriTVOL: UITableView!
    
   
    override func viewDidLoad() {
        super.viewDidLoad()
        // Do any additional setup after loading the view.
        KolukuluriTVOL.dataSource = self
        KolukuluriTVOL.delegate = self
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if transition == "KolukuluriDescriptionSegue"{
            let destination = segue.destination as! KolukuluriGameController
            destination.game = games[(KolukuluriTVOL.indexPathForSelectedRow?.row)!]
        }
    }
